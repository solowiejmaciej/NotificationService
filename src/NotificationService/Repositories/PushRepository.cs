...
public interface IPushRepository : INotificationRepository
{
    ...
    Task<List<PushNotification>>
        GetAllPushesToUserIdAsync(string userId, CancellationToken cancellationToken = default);
...
public sealed class PushRepository : IPushRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        return await _dbContext.PushNotifications.Where(
            e => e.RecipientId == userId && e.Status != EStatus.ToBeDeleted).ToListAsync(cancellationToken);
    }
